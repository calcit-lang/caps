...
use crate::{PackageDeps, call_build_script, module_folder};
...
"#;
    fs::write(cache_root.join("AGENTS.md"), content)
        .map_err(|e| format!("failed to write {}/AGENTS.md: {e}", cache_root.display()))
}
    ...
    for owner in read_directories(&git_root)? {
        for repository in read_directories(&owner)? {
            ...
            for revision in revisions {
                ...
                fs::remove_dir_all(&revision.path).map_err(|e| {
                    format!(
                        "failed to remove old module revision {}: {e}",
...
impl CacheMetadataLock {
    ...
    fn acquire_named(lock_root: &Path, file_name: &str) -> Result<Self, String> {
        fs::create_dir_all(lock_root)
            .map_err(|e| format!("failed to create {}: {e}", lock_root.display()))?;
...
fn metadata_cache_root(source: &Path) -> PathBuf {
    for ancestor in source.ancestors() {
                ...
                == Some(MODULE_CACHE_DIR)
        {
            return ancestor
                .parent()
                .expect("git cache directory has a parent")
                .to_path_buf();
        }
    }
...
fn register_project_view(options: &GraphOptions) -> Result<(), String> {
    ...
    fs::create_dir_all(&registry)
        .map_err(|e| format!("failed to create {}: {e}", registry.display()))?;
...
fn active_project_view_targets(modules_dir: &Path) -> Result<Vec<PathBuf>, String> {
        ...
        .map_err(|e| format!("failed to read {}: {e}", registry.display()))?
    {
        ...
        for module in fs::read_dir(&modules)
            .map_err(|e| format!("failed to read {}: {e}", modules.display()))?
        {
...
fn revision_is_linked_by_project_view(revision: &Path, active_targets: &[PathBuf]) -> bool {
    ...
    active_targets
        .iter()
        .any(|target| target.starts_with(&revision))
}
...
pub fn resolve_graph(
    ...
    let pass = stable_result
        .ok_or_else(|| "dependency resolution did not converge after 256 passes".to_string())?;
    ...
    if options.strict && !warnings.is_empty() {
        return Err(format!(
            "strict dependency resolution rejected warnings:\n{}",
            warnings.join("\n")
        ));
    }
...
fn resolve_pass(
    ...
    let selected_items = previous
        ...
        .chain(
            root.iter()
                .filter(|(repository, _)| !previous.contains_key(*repository)),
        )
    ...
    for batch in missing.chunks(MAX_PARALLEL_RESOLVES) {
        ...
        for handle in handles {
            let (repository, selected, result) = handle.join().map_err(|_| {
                "dependency resolver worker panicked while inspecting a module".to_string()
            })?;
            let module = result?;
...
fn select_reference(
    ...
    let versions = refs
        .iter()
        .map(|reference| {
            parse_tag_version(reference).map(|version| ((*reference).to_string(), version))
        })
    ...
    if !published_versions.is_empty() {
        ...
        return Ok(published_versions
            .last()
            .expect("non-empty published versions")
            .0
            .clone());
    }
...
fn materialize_module(
    ...
    fs::create_dir_all(&temp_root)
        .map_err(|e| format!("failed to create {}: {e}", temp_root.display()))?;
    ...
    if temp_path.exists() {
        fs::remove_dir_all(&temp_path)
            .map_err(|e| format!("failed to clean {}: {e}", temp_path.display()))?;
    }
    ...
    }
    {
        ...
        if source.exists() {
            ...
            let parent = source
                .parent()
                .ok_or_else(|| format!("invalid store path {}", source.display()))?;
            ...
            if let Err(error) = fs::rename(&temp_path, &source) {
                if source.exists() {
                    fs::remove_dir_all(&temp_path).map_err(|e| {
                        format!(
                            "failed to clean {} after concurrent install: {e}",
                            temp_path.display()
                        )
                    })?;
                    validate_store_source(repository, &source, &commit)?;
...
fn ensure_store_metadata_unlocked(
    source: &Path,
    repository: &str,
    reference: &str,
    commit: &str,
    ...
    let retained_reference = fs::read_to_string(&metadata)
        ...
        .map_or_else(
            || reference.to_string(),
            |existing| preferred_store_reference(&existing, reference),
        );
    fs::write(
        &metadata,
...
fn inspect_remote_ref(url: &str, reference: &str) -> Result<RemoteRef, String> {
    ...
    let tag_commit = refs
        .get(peeled_tag_ref.as_str())
        .or_else(|| refs.get(tag_ref.as_str()));
...
fn read_module_dependencies(source: &Path) -> Result<BTreeMap<String, String>, String> {
    ...
    let parsed = cirru_edn::parse(&content)
        .map_err(|e| format!("failed to parse {}: {e}", deps_path.display()))?;
    let deps: PackageDeps = parsed
        .try_into()
        .map_err(|e| format!("invalid {}: {e}", deps_path.display()))?;
    Ok(sorted_dependencies(&deps.dependencies))
}

fn sorted_dependencies(
    deps: &std::collections::HashMap<Arc<str>, Arc<str>>,
...
fn validate_module_folders<'a>(
    repositories: impl Iterator<Item = &'a String>,
) -> Result<(), String> {
...
fn collect_warnings(
    ...
    for (repository, module) in modules {
        match module.kind {
            ...
            RefKind::Tag if parse_tag_version(&module.reference).is_none() => {
        ...
        if let Some(module_requests) = requests.get(repository) {
            let distinct = module_requests
                .iter()
                .map(|request| &request.reference)
                .collect::<BTreeSet<_>>();
            if distinct.len() > 1 {
                let reason = if root.get(repository) == Some(&module.reference)
                    && distinct
                        .iter()
                        .any(|value| parse_tag_version(value).is_none())
                {
                    "root override"
                } else if distinct
                    .iter()
...
fn install_project_view_with<F>(
    graph: &ResolvedGraph,
    options: &GraphOptions,
    register: F,
) -> Result<(), String>
    ...
    F: Fn(&GraphOptions) -> Result<(), String>,
{
    ...
    if next_modules.exists() {
        fs::remove_dir_all(&next_modules)
            .map_err(|e| format!("failed to clean {}: {e}", next_modules.display()))?;
    }
    fs::create_dir_all(&next_modules)
        .map_err(|e| format!("failed to create {}: {e}", next_modules.display()))?;
    ...
    if backup.exists() {
        fs::remove_dir_all(&backup)
            .map_err(|e| format!("failed to clean {}: {e}", backup.display()))?;
    }
    if fs::symlink_metadata(&modules_path).is_ok() {
        fs::rename(&modules_path, &backup)
            .map_err(|e| format!("failed to stage old module view: {e}"))?;
    }
    ...
    if state_backup.exists() {
        fs::remove_file(&state_backup)
            .map_err(|e| format!("failed to clean {}: {e}", state_backup.display()))?;
    }
    ...
    if let Err(error) = write_state(graph, &view_modes, &state_path) {
        fs::remove_dir_all(&modules_path)
            .map_err(|e| format!("{error}; also failed to remove new module view: {e}"))?;
        if backup.exists() {
            fs::rename(&backup, &modules_path)
                .map_err(|e| format!("{error}; also failed to restore old module view: {e}"))?;
        }
        if state_backup.exists() {
            fs::rename(&state_backup, &state_path)
                .map_err(|e| format!("{error}; also failed to restore old caps state: {e}"))?;
        }
        return Err(error);
    }
    if let Err(error) = register(options) {
        ...
        if fs::symlink_metadata(&state_path).is_ok() {
            fs::remove_file(&state_path)
                .map_err(|e| format!("{error}; also failed to remove new caps state: {e}"))?;
        }
        if state_backup.exists() {
            fs::rename(&state_backup, &state_path)
                .map_err(|e| format!("{error}; also failed to restore old caps state: {e}"))?;
    ...
    if state_backup.exists() {
        fs::remove_file(&state_backup)
            .map_err(|e| format!("failed to clean old caps state: {e}"))?;
    }
    Ok(())
}
...
pub fn check_project_view(graph: &ResolvedGraph, options: &GraphOptions) -> Result<usize, String> {
    ...
    for module in graph.modules.values() {
        ...
        let actual = fs::canonicalize(&link)
            .map_err(|e| format!("failed to resolve {}: {e}", link.display()))?;
        let expected_target = expected_link_target(module, &options.calcit_version)?;
        ...
        let expected = fs::canonicalize(&expected_target)
            .map_err(|e| format!("failed to resolve {}: {e}", expected_target.display()))?;
        if actual != expected {
            ...
            println!(
                "{}",
                format!("√ {} at {}", module.repository, module.reference).dimmed()
            );
        }
    }
    Ok(issues)
    ...
    for module in graph.modules.values() {
        ...
        if commit != module.commit {
            println!(
                "{}",
                format!(
                    "! {} store commit is {commit}, expected {}",
        ...
        if !changes.is_empty() {
            println!(
                "{}",
                format!(
                    "! {} immutable source has local changes: {}",
                    module.repository,
                    changes.join(", ")
                )
                .red()
        ...
        if expected_target != module.source {
            match verify_native_receipt(module, &expected_target, &options.calcit_version) {
                Err(error) => {
                    println!(
                        "{}",
                        format!("! {} native receipt is invalid: {error}", module.repository).red()
                    );
                    issues += 1;
                }
                Ok(libraries) => {
                    ...
                    if libraries.is_empty() {
                        println!(
                            "{}",
                            format!(
                                "! {} has no .{extension} library under dylibs/",
                                module.repository
                            )
                            .red()
                        );
                    ...
                    for library in libraries {
                        let output =
                            Command::new(std::env::current_exe().map_err(|e| e.to_string())?)
                                ...
                                .map_err(|e| {
                                    format!(
                                        "failed to start native verifier for {}: {e}",
                                        library.display()
                                    )
                                })?;
                        if !output.status.success() {
...
pub fn verify_native_library(path: &Path) -> Result<(), String> {
    ...
    for (name, symbol, expected) in [
        (
            "buffer",
            calcit_native_ffi::BUFFER_PROTOCOL_VERSION_SYMBOL,
            calcit_native_ffi::BUFFER_PROTOCOL_VERSION,
        ),
        (
            "async",
            calcit_native_ffi::ASYNC_PROTOCOL_VERSION_SYMBOL,
            calcit_native_ffi::ASYNC_PROTOCOL_VERSION,
        ),
        (
            "resource",
            calcit_native_ffi::RESOURCE_PROTOCOL_VERSION_SYMBOL,
            calcit_native_ffi::RESOURCE_PROTOCOL_VERSION,
        ),
    ] {
    ...
    if advertised.contains(&"buffer")
        && unsafe {
            library.get::<calcit_native_ffi::BufferFreeFn>(calcit_native_ffi::BUFFER_FREE_SYMBOL)
        }
        .is_err()
    {
        return Err(format!(
    ...
    if advertised.contains(&"resource")
        && unsafe {
            library.get::<calcit_native_ffi::ResourceReleaseV1>(
                calcit_native_ffi::RESOURCE_RELEASE_SYMBOL,
            )
        }
        .is_err()
    {
...
fn write_state(
    graph: &ResolvedGraph,
    view_modes: &BTreeMap<String, ProjectModuleMode>,
    state_path: &Path,
) -> Result<(), String> {
    ...
    for (repository, module) in &graph.modules {
        ...
        info.insert(
            Edn::tag("path"),
            Edn::str(module.link_target.to_string_lossy().as_ref()),
        );
    ...
    fs::rename(&temp, state_path)
        .map_err(|e| format!("failed to activate {}: {e}", state_path.display()))
}

fn prepare_native_realization(
    ...
    let realization = expected_link_target(module, &options.calcit_version)?;
    let build_key = realization
        .file_name()
        .and_then(|name| name.to_str())
        .unwrap_or("unknown");
    let receipt = realization.join(".calcit-native.cirru");
    if receipt.exists() {
        verify_native_receipt(module, &realization, &options.calcit_version)?;
    ...
    fs::create_dir_all(realization_parent)
        .map_err(|e| format!("failed to create {}: {e}", realization_parent.display()))?;
    let temp = realization_parent.join(format!(
        ".tmp-native-{}-{}",
        std::process::id(),
        module_folder(&module.repository)?
    ));
    if temp.exists() {
        fs::remove_dir_all(&temp)
            .map_err(|e| format!("failed to clean {}: {e}", temp.display()))?;
    }
    ...
    {
        return Err(format!(
            "native module {} did not produce files under dylibs/",
            module.repository
        ));
    }
    write_native_receipt(module, &temp, build_key, &options.calcit_version)?;
    if realization.exists() {
        ...
        if realization.exists() {
            fs::remove_dir_all(&temp).map_err(|e| {
                format!(
                    "failed to clean {} after concurrent native build: {e}",
                    temp.display()
                )
            })?;
        } else {
...
fn expected_link_target(module: &ResolvedModule, calcit_version: &str) -> Result<PathBuf, String> {
    if !module.source.join("build.sh").exists() {
        return Ok(module.source.clone());
    }
...
fn write_native_receipt(
    ...
    receipt.insert(
        Edn::tag("ffi-buffer"),
        Edn::str(calcit_native_ffi::BUFFER_PROTOCOL_VERSION.to_string()),
    );
    receipt.insert(
        Edn::tag("ffi-async"),
        Edn::str(calcit_native_ffi::ASYNC_PROTOCOL_VERSION.to_string()),
    );
    receipt.insert(
        Edn::tag("ffi-resource"),
        Edn::str(calcit_native_ffi::RESOURCE_PROTOCOL_VERSION.to_string()),
    );
    ...
    fs::write(realization.join(".calcit-native.cirru"), content)
...
fn verify_native_receipt(
    ...
    let receipt = cirru_edn::parse(&content)
        ...
        .view_map()?;
    let expected_build_key = realization
        .file_name()
        .and_then(|name| name.to_str())
        .unwrap_or("unknown");
    let buffer_version = calcit_native_ffi::BUFFER_PROTOCOL_VERSION.to_string();
    let async_version = calcit_native_ffi::ASYNC_PROTOCOL_VERSION.to_string();
    let resource_version = calcit_native_ffi::RESOURCE_PROTOCOL_VERSION.to_string();
    for (key, expected) in [
        ...
        if actual != expected {
            return Err(format!(
                "receipt :{key} is {actual:?}, expected {expected:?}"
            ));
        }
    }
    ...
    if declared.len()
        != receipt
            .tag_get("artifacts")
            .expect("artifacts was read above")
            .view_list()?
            .0
            .len()
    {
    ...
    Ok(actual
        .keys()
        .map(|relative_path| realization.join(relative_path))
        .collect())
}
...
fn native_artifact_from_edn(value: &Edn) -> Result<NativeArtifact, String> {
    ...
    if size < 0.0 || size.fract().abs() > f64::EPSILON || size > u64::MAX as f64 {
        return Err(format!(
            "receipt artifact size must be a non-negative integer, got {size}"
        ));
    }
    let md5 = map
        .tag_get("md5")
...
fn collect_native_artifacts(
    realization: &Path,
) -> Result<BTreeMap<String, NativeArtifact>, String> {
    let root = fs::canonicalize(realization)
        .map_err(|e| format!("failed to resolve {}: {e}", realization.display()))?;
    let dylibs = realization.join("dylibs");
    let dylibs_metadata = fs::symlink_metadata(&dylibs)
        .map_err(|e| format!("native realization is missing {}: {e}", dylibs.display()))?;
    ...
    if artifacts.is_empty() {
        return Err(format!(
            "native realization has no artifacts under {}",
            dylibs.display()
        ));
    }
    Ok(artifacts)
...
fn collect_native_artifact_dir(
        ...
        .map_err(|e| format!("failed to read {}: {e}", directory.display()))?
    {
        ...
        if metadata.file_type().is_symlink() {
            return Err(format!(
                "native artifact {} must not be a symlink",
                path.display()
            ));
        }
        if metadata.is_dir() {
            ...
            let relative_path = path
                ...
                .map_err(|e| {
                    format!(
                        "failed to make artifact path relative for {}: {e}",
                        path.display()
                    )
                })?
                .to_string_lossy()
            ...
            return Err(format!(
                "native artifact {} has an unsupported file type",
                path.display()
            ));
        }
    }
...
fn print_tree_module(
    ...
    for (index, dependency) in module.dependencies.keys().enumerate() {
        print_tree_module(
            ...
            dependency,
            &next_prefix,
            index + 1 == module.dependencies.len(),
            seen,
        );
    }
}
...
pub fn print_why(graph: &ResolvedGraph, target: &str) -> Result<(), String> {
    if !graph.modules.contains_key(target) {
        return Err(format!(
            "module {target} is not in the resolved dependency graph"
        ));
    }
...
mod tests {
    use super::{
        DependencyRequest, GraphOptions, RefKind, ResolvedGraph, ResolvedModule,
        clean_version_store, ensure_store_metadata, install_project_view_with, is_full_commit_hash,
        parse_tag_version, read_module_dependencies, select_reference, sorted_root_dependencies,
        verify_native_receipt, write_modules_agents, write_native_receipt,
    };
    ...
    fn incomparable_refs_need_root_decision() {
        let requests = BTreeSet::from([
            request("main", Some("a@1.0.0")),
            request("next", Some("b@1.0.0")),
        ]);
    ...
    fn published_tag_wins_over_transitive_branch() {
        let requests = BTreeSet::from([
            request("0.0.6", Some("stable@1.0.0")),
            request("main", Some("legacy@1.0.0")),
        ]);
        assert_eq!(
            select_reference("org/repo", &requests, None),
            Ok("0.0.6".to_string())
        );
    }
    ...
    fn transitive_module_excludes_development_dependencies() {
        let root =
            std::env::temp_dir().join(format!("calcit-caps-transitive-dev-{}", std::process::id()));
    ...
    .unwrap();
        ...
        assert_eq!(
            dependencies.get("org/runtime").map(String::as_str),
            Some("1.0.0")
        );
    ...
    fn native_receipt_rejects_changed_artifacts() {
        ...
        write_native_receipt(&module, &realization, "build-key", "0.13.72").unwrap();
        assert_eq!(
            verify_native_receipt(&module, &realization, "0.13.72").unwrap(),
            vec![library.clone()]
        );
        fs::write(&library, b"modified build").unwrap();
        assert!(verify_native_receipt(&module, &realization, "0.13.72").is_err());
        fs::remove_dir_all(root).unwrap();
    }
    ...
    fn clean_keeps_revisions_linked_by_registered_project_views() {
        let root =
            std::env::temp_dir().join(format!("calcit-caps-clean-linked-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        let modules_dir = root.join("modules");
    ...
    fn store_metadata_retains_the_highest_observed_semver_reference() {
        let root =
            std::env::temp_dir().join(format!("calcit-caps-metadata-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
    ...
    fn concurrent_store_metadata_updates_retain_the_highest_semver_reference() {
        let root = std::env::temp_dir().join(format!(
            "calcit-caps-metadata-concurrent-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&root);
        let source = root.join("revision/source");
    ...
    fn project_view_registration_failure_restores_previous_view_and_state() {
        let root =
            std::env::temp_dir().join(format!("calcit-caps-view-rollback-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        ...
        let options = GraphOptions {
            ...
            calcit_version: "0.13.72".to_string(),
            ci: false,
            strict: false,
            build_native: false,
        };
        let result =
            install_project_view_with(&graph, &options, |_| Err("registration failed".to_string()));
        assert_eq!(result.unwrap_err(), "registration failed");
        assert_eq!(
            fs::read_to_string(modules_path.join("old.txt")).unwrap(),
            "old view"
        );
        assert_eq!(
            fs::read_to_string(calcit_dir.join("caps-state.cirru")).unwrap(),
            "old state"
        );
        fs::remove_dir_all(root).unwrap();
    }
