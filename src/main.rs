...
fn parse_dependency_group(
    deps_info: &cirru_edn::EdnMapView,
    field: &str,
) -> Result<HashMap<Arc<str>, Arc<str>>, String> {
...
pub fn main() -> Result<(), String> {
    ...
    if raw_args.get(1).is_some_and(|arg| arg == "__verify-native") {
        let path = raw_args
            .get(2)
            .ok_or_else(|| "missing native library path".to_string())?;
    ...
    if cli_args.pull_branch {
        eprintln!(
            "[Warn] --pull-branch is deprecated; branch refs are always resolved from the remote"
        );
    }
    ...
    if Path::new(&cli_args.input).exists() {
        ...
        if let Some(expected) = &deps.calcit_version {
            match detected_calcit_version(&cli_args) {
                Ok(detected) if expected != &detected.version => eprintln!(
                    "[Warn] calcit version mismatch, deps.cirru expected {expected}, detected {} from {}",
                    detected.version, detected.source
                ),
                Err(error) => {
                    eprintln!("[Warn] unable to verify deps.cirru :calcit-version: {error}")
                }
                _ => {}
            }
        }
        ...
        match &cli_args.subcommand {
            Some(SubCommand::Outdated(opts)) => {
                let calcit_version = detected_calcit_version(&cli_args)?.version;
                let updated = outdated_tags(deps, &cli_args.input, opts.yes, &calcit_version)?;
                if updated {
            ...
            Some(SubCommand::Upgrade(opts)) => {
                let calcit_version = detected_calcit_version(&cli_args)?.version;
                let updated = upgrade_packages(deps, &cli_args.input, opts, &calcit_version)?;
                if updated {
                    // Re-read deps.cirru and download updated dependencies
                    println!("\nDownloading updated dependencies...");
                    let content = fs::read_to_string(&cli_args.input).map_err(|e| e.to_string())?;
            ...
            Some(SubCommand::Add(opts)) => {
                ...
                for raw in &opts.packages {
                    ...
                    let (package, inline_version) =
                        split_at.map_or((raw.as_str(), None), |index| {
                    ...
                    target.insert(
                        org_and_folder.into(),
                        inline_version.unwrap_or(&opts.version).to_owned().into(),
                    );
                }
            ...
            Some(SubCommand::Status(_)) => {
                ...
                let issues = check_project_view(&graph, &graph_options(&deps, &cli_args, false)?)?;
                if issues > 0 {
            ...
            Some(SubCommand::Verify(opts)) => {
                ...
                let graph = resolve_for_cli(&deps, &cli_args, false)?;
                print_warnings(&graph);
                let issues = verify_project_view(&graph, &graph_options(&deps, &cli_args, false)?)?;
                if issues > 0 {
            ...
            Some(SubCommand::Reset(_)) => {
                ...
                print_warnings(&graph);
                let graph_options = graph_options(&deps, &cli_args, true)?;
...
fn graph_options(
    ...
    let calcit_version = deps
        .calcit_version
        .clone()
        .map(Ok)
        .unwrap_or_else(|| detected_calcit_version(options).map(|detected| detected.version))?;
    Ok(GraphOptions {
        project_root,
...
fn project_root_from_input(input: &str) -> Result<PathBuf, String> {
    ...
    absolute_input
        ...
        .ok_or_else(|| {
            format!(
                "cannot determine project directory from {}",
...
fn verify_toolchain(deps: &PackageDeps, options: &TopLevelCaps) -> Result<(), String> {
    ...
    Version::parse(expected)
        .map_err(|error| format!("invalid :calcit-version '{expected}': {error}"))?;
    let detected = detected_calcit_version(options)?;
    if expected != detected.version {
        return Err(format!(
            "Calcit toolchain mismatch: {} declares {expected}, but {} reports {}; install Calcit {expected} or upgrade deps.cirru",
            options.input, detected.source, detected.version
        ));
    }

    let project_root = graph_options(deps, options, false)?.project_root;
...
fn read_procs_manifest_spec(package_json: &Path) -> Result<Option<String>, String> {
    ...
    let package: serde_json::Value = serde_json::from_str(&content)
        .map_err(|error| format!("invalid {}: {error}", package_json.display()))?;
...
fn resolve_for_cli(
    ...
    resolve_graph(deps, &graph_options(deps, options, build_native)?)
}
...
pub const CAPS_VERSION: &str = env!("CARGO_PKG_VERSION");

#[derive(Debug, Clone, PartialEq, Eq)]
struct DetectedCalcitVersion {
    version: String,
    source: String,
}

fn detected_calcit_version(options: &TopLevelCaps) -> Result<DetectedCalcitVersion, String> {
    if let Some(version) = &options.calcit_version {
        Version::parse(version)
            .map_err(|error| format!("invalid --calcit-version '{version}': {error}"))?;
        return Ok(DetectedCalcitVersion {
            version: version.clone(),
            source: "--calcit-version override".to_string(),
        });
    }

    let mut failures = Vec::new();
    for argument in ["--version", "-v"] {
        match Command::new("calcit").arg(argument).output() {
            Ok(output) if output.status.success() => {
                ...
                if let Some(version) = parse_calcit_version_output(&text) {
                    return Ok(DetectedCalcitVersion {
                        version,
                        source: format!("`calcit {argument}` on PATH"),
                    });
                }
                failures.push(format!("`calcit {argument}` returned no SemVer"));
            }
            Ok(output) => {
                failures.push(format!("`calcit {argument}` exited with {}", output.status))
            }
            Err(error) => failures.push(format!("failed to run `calcit {argument}`: {error}")),
        }
    }
    Err(format!(
        "could not detect the Calcit toolchain version ({}); install calcit or pass --calcit-version <version> from a verified CI/embedding environment",
        failures.join("; ")
    ))
}

fn parse_calcit_version_output(output: &str) -> Option<String> {
    output
        .split_whitespace()
        .map(|token| {
...
struct TopLevelCaps {
    ...
    /// print the caps package version and exit
    #[argh(switch)]
    version: bool,

    /// verified Calcit toolchain version override for CI or embedding
    #[argh(option)]
    calcit_version: Option<String>,
...
fn normalize_package_name(raw: &str) -> Result<String, String> {
    ...
    let (org, repo) = validated_module_parts(&s).map_err(|_| {
        format!("invalid package '{raw}', expected canonical org/repo or github URL")
    })?;
    Ok(format!("{org}/{repo}"))
}
...
fn validated_module_parts(name: &str) -> Result<(&str, &str), String> {
    ...
    let org = segments
        .next()
        .ok_or_else(|| format!("invalid module name '{name}'"))?;
    let repo = segments
        .next()
        .ok_or_else(|| format!("invalid module name '{name}'"))?;
    if segments.next().is_some() || !valid_module_component(org) || !valid_module_component(repo) {
        return Err(format!(
            "invalid module name '{name}', expected canonical org/repo"
        ));
    }
    Ok((org, repo))
}
...
fn outdated_tags(
    ...
    let calcit_version_needs_update = match old_calcit_version {
        ...
        Some(version) => match (
            ...
            Version::parse(calcit_version).ok(),
        ) {
        ...
        if calcit_version_needs_update {
            match old_calcit_version {
                None => changes.push(format!(
                    "calcit-version missing, will set to {calcit_version}"
                )),
...
fn show_package_versions(
    org_and_folder: Arc<str>,
    version: Arc<str>,
    ...
    if let Some((latest_tag, latest_version)) = latest {
        ...
        if current
            .as_ref()
            .is_ok_and(|current| current < &latest_version)
        {
            print_column(
                org_and_folder.yellow(),
                version.yellow(),
                latest_tag.yellow(),
                "Outdated".yellow(),
            );
...
fn upgrade_packages(
    ...
    if !outdated_packages.is_empty() || calcit_version_needs_update {
        ...
        if opts.all {
            sync_calcit_procs_package(&project_root_from_input(deps_file)?, calcit_version)?;
        }
        println!("deps.cirru updated successfully!");
        Ok(true)
    } else {
...
fn sync_calcit_procs_package(project_root: &Path, calcit_version: &str) -> Result<(), String> {
    let package_json = project_root.join("package.json");
    if !package_json.exists() {
        println!(
    ...
    match &opts.command {
        ...
        VersionSubcommand::Set(opts) => {
            Version::parse(&opts.version)
                .map_err(|e| format!("invalid SemVer version '{}': {e}", opts.version))?;
        ...
        .ok_or_else(|| format!("no :version declared in {deps_file}; initialize it with `caps version set <version> {deps_file}`"))?;
            ...
            match opts.level.as_str() {
                ...
                other => {
                    return Err(format!(
                        "invalid bump level '{other}', expected major, minor, or patch"
                    ));
                }
            }
...
fn write_package_version(deps_file: &str, version: &str) -> Result<(), String> {
    let content = fs::read_to_string(deps_file).map_err(|e| e.to_string())?;
    let mut parsed =
        cirru_edn::parse(&content).map_err(|e| format!("failed to parse {deps_file}: {e}"))?;
...
fn print_column(
    pkg: ColoredString,
    expected: ColoredString,
    latest: ColoredString,
    hint: ColoredString,
) {
...
mod tests {
    ...
    fn module_names_are_canonical_before_path_use() {
        assert_eq!(
            module_folder("calcit-lang/respo.calcit"),
            Ok("respo.calcit")
        );
        for invalid in [
            "../outside",
    ...
    #[test]
    fn parses_plain_and_labelled_calcit_versions() {
        assert_eq!(
            parse_calcit_version_output("0.13.70\n"),
            Some("0.13.70".to_string())
        );
        assert_eq!(
            parse_calcit_version_output("calcit version: 0.13.70\n"),
            Some("0.13.70".to_string())
        );
        assert_eq!(
    ...
    fn missing_dependencies_field_is_an_empty_graph() {
        let deps: PackageDeps = Edn::Map(cirru_edn::EdnMapView::default())
            .try_into()
            .unwrap();
    ...
    fn reads_procs_spec_from_runtime_or_development_dependencies() {
        let root =
            std::env::temp_dir().join(format!("calcit-caps-toolchain-{}", std::process::id()));
        ...
        std::fs::write(
            &package_json,
            r#"{"dependencies":{"@calcit/procs":"^0.13.38"}}"#,
        )
        .unwrap();
        assert_eq!(
            super::read_procs_manifest_spec(&package_json),
            Ok(Some("^0.13.38".to_owned()))
        );

        std::fs::write(
            &package_json,
            r#"{"devDependencies":{"@calcit/procs":"0.13.38"}}"#,
        )
        .unwrap();
        assert_eq!(
            super::read_procs_manifest_spec(&package_json),
    ...
    fn toolchain_manifest_and_installed_versions_require_exact_matches() {
        let root = std::env::temp_dir().join(format!(
            "calcit-caps-toolchain-enforcement-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn version_commands_do_not_read_snapshot_version() {
        ...
        let deps: PackageDeps = cirru_edn::parse("{} (:dependencies $ {})")
            .unwrap()
            .try_into()
            .unwrap();
        ...
        assert!(
            get_error.contains("no :version declared in"),
            "unexpected error: {get_error}"
        );
        assert!(
            get_error.contains("deps.cirru"),
            "unexpected error: {get_error}"
        );
        assert!(
        ...
        let bump_error = handle_version_command(
            ...
            deps_file,
            &VersionCaps {
                command: VersionSubcommand::Bump(VersionBumpCaps {
                    level: "patch".to_owned(),
                }),
            },
        )
        .expect_err("missing deps.cirru version should reject version bump");
        assert!(
            bump_error.contains("no :version declared in"),
            "unexpected error: {bump_error}"
        );
        assert!(
            bump_error.contains("deps.cirru"),
            "unexpected error: {bump_error}"
        );
        assert!(
    ...
    fn parses_development_dependencies_separately() {
        ...
        let deps: PackageDeps = parsed.try_into().unwrap();
        assert_eq!(
            deps.dependencies.get("org/runtime").map(AsRef::as_ref),
            Some("1.0.0")
        );
        assert_eq!(
            deps.dev_dependencies.get("org/test").map(AsRef::as_ref),
            Some("main")
        );
    }
    ...
    fn write_deps_preserves_package_version() {
        let path =
            std::env::temp_dir().join(format!("calcit-caps-version-{}.cirru", std::process::id()));
